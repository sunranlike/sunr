...
func main() {
	...
	registerRouter(core)        //注册core,目的是为了绑定foo与FooControllerHandler,让他们一个作为map一个作为value
	server := &http.Server{     //这个结构体的Handler字段就只去使用自己的Handler函数
		// 自定义的请求核心处理函数
		Handler: core, //Handler是一个http.Serve的接口,这个接口的方法集合只有一个方法:serveHTTP
		//如果你没有实现这个serveHTTP方法,就会使用默认的defaultServeHTTP方法,也就是我们的core需要一个有一个方法叫serveHTTP
		//我们也确实实现了这个接口
		// 请求监听地址
		Addr: ":8080",
	}
