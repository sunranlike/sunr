...
//他会return一个方法,这个方法干了什么?首先新来了三个chan,一个是通知时间到了:durationCtx
//一个是错误了:panicChan
//一个是执行完了: finish
//整个逻辑就是开一个routine执行原本的逻辑,然后select+chan 非阻塞io监听三个chan
//无论哪个chan执行都会终止代码(因为要么设定的时间到了,要么是err了,要么是finish了)
