...
var newCommand = &cobra.Command{
	...
	RunE: func(c *cobra.Command, args []string) error {
		...
		// 获取folder下的sunranlike-hade-xxx相关解压目录
		...
		filepath.Walk(folder, func(path string, info os.FileInfo, err error) error {
			...
			if path == filepath.Join(folder, "go.mod") {
				...
				c = bytes.ReplaceAll(c, []byte("module github.com/sunranlike/hade"), []byte("module "+mod))
				c = bytes.ReplaceAll(c, []byte("require ("), []byte("require (\n\tgithub.com/sunranlike/hade "+version))
			...
			isContain := bytes.Contains(c, []byte("github.com/sunranlike/hade/app"))
			if isContain {
				fmt.Println("更新文件:" + path)
				c = bytes.ReplaceAll(c, []byte("github.com/sunranlike/hade/app"), []byte(mod+"/app"))
				err = ioutil.WriteFile(path, c, 0644)
				if err != nil {
					return err
				}
